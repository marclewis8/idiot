...
(defn hash-object [dir dbase args]
  ...
  (cond
    (= (count args) 0) "Error: you must specify a file."
    (= (count args) 1) (let [[f] args]
                         (cond
                           (or (= f "-h") (= f "--help"))  hmsg/hash-h-message
                           (= f "-w") "Error: you must specify a file." ; special case
                           (not (.isDirectory (io/file (str dir File/separator dbase)))) "Error: could not find database. (Did you run `idiot init`?)"
                           (not (.exists (io/file (str dir File/separator f)))) "Error: that file isn't readable"
                           :else
                           (let [content (slurp (str dir File/separator f))
                             ...
                             ;(println address)
                             (str address))))
    (= (count args) 2) (let [[flag file] args]
                         (cond
                           (or (= flag "-h") (= flag "--help")) hmsg/hash-h-message ; return it, it will get printed
                           (not (.isDirectory (io/file (str dir File/separator dbase)))) "Error: could not find database. (Did you run `idiot init`?)"
                           (not= flag "-w") "Error: invalid flag given"
                           (not (.exists (io/file (str dir File/separator file)))) "Error: that file isn't readable"
                           :else
