...
(declare write-object commit-logic)
              ...
              (if (= (count (rest more)) 0)
                ...
                (loop [parent-entries ""
                  ...
                  (if (= (count parent-list) 0)
                    ...
                    ; Add more parents
                    (let [pname-abb (first (rest parent-list))]
                      (if (nil? pname-abb)
                        (println "Error: you must specify a commit object with the -p switch.")
                        (let [pname (tool/abbrev-to-full-hash dir dbase pname-abb)
                              pdir (subs pname 0 2)
                              pfname (subs pname 2)]
...
(defn commit-logic [dir dbase commit-addr]
  (let [refs-path (str dir File/separator dbase File/separator "refs" File/separator "heads" File/separator)
        head-path (str dir File/separator dbase File/separator "HEAD" File/separator)
        head-contents (slurp (io/file head-path))
